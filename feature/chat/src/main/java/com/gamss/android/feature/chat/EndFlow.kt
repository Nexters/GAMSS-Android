...
sealed interface EndFlow {
     ...
     *
     * 순수한 연출 값이지만 컴포지션이 아니라 여기 두는 이유가 있다. 카드는 이미 서버에 만들어져
     * 되돌릴 수 없어서, 프로세스가 죽었다 살아나도 접다 만 자리에서 이어져야 한다. 화면 쪽
     * rememberSaveable 로는 ViewModel 이 복원한 카드와 단계가 따로 놀 수 있다.
     */
