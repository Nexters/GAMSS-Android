 ...
 * 갱신하고 [alerts]로 흘려보낸다. LOW(10% 미만 남음)는 하루 1회만, EXHAUSTED(전부 소진)는 소진
 * 상태로 재조회될 때마다 매번 흘려보낸다 — 두 알림의 발행 정책이 다르다. 여러 채팅방을 오가도(대화방을
...
interface TokenUsageRefreshNotifier {
    ...
    /**
     * 가장 최근 조회에서 서버가 확정한 소진 여부. 화면은 이 값으로 전송 버튼을 잠근다 —
